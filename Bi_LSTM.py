...
class BiLSTM(nn.Module):
    def __init__(self, args):
        ...
        self.word_embedding = Embedding.Embedding(args.embed_num, args.embedding_dim)
        ...
        if self.use_pretrained_emb:
            ...
            self.word_embedding_static = Embedding.ConstEmbedding(pretrained_weight)

        self.lstm = nn.LSTM(args.embedding_dim*2, args.hidden_dim, bidirectional=True, dropout=args.dropout_model)
    ...
    def forward(self, sentence):
        ...
        y = self.word_embedding.forward(sentence)           # [torch.FloatTensor of size 49x64x300]
        # print(y)
        x = torch.cat([x, y], 2)
